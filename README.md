...
- **Player Names:** When a player achieves a top-10 score, they are prompted to enter their name
...
- Mazes are represented as 2D arrays where cell value `15` indicates a wall
...
```
pac_man/
├── main.py                      # Entry point and theme initialization
...
1. **Startup:** `main.py` loads theme → creates `Visualizer` → renders `MainMenuScene`
